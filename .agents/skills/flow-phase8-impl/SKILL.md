...
description: "新規機能開発フロー Phase 8（実装・Green Phase）。実装ワーカー役割（impl-builder）のサブエージェントを fresh context で起動し、Phase 7 で承認済みのテストを通す最小限の実装を行わせる。テストの修正は禁止。司令塔（メイン会話）はゲートの独立検証と Green Phase 証拠の記録を担当する。完了後は次フェーズ（Phase 9 リファクタ or Phase 10 実装検証）を案内する。使用トリガー: 「実装して」「Green Phase に入って」「Phase 8 を実行」「テストを通して」等。"
---

# Phase 8: 実装（Green Phase）

新規機能開発の標準10フェーズフロー（`docs/開発プロセス/設計から実装までのフロー.md` 参照）の **Phase 8** を担当するスキル。Phase 7 で人間が承認したテストを通す実装を、司令塔（メイン会話のAI）と分離された **fresh context** の `impl-builder` ワーカーエージェントが行う。
...
3. `docs/設計/<機能名>/_flow-config.md` — フロー設定（**Phase 7 の実施要否**・実装承認ゲートの実施要否）
4. `docs/設計/<機能名>/_review/test/findings.md` — **Phase 7 の承認判定記録**（またはスキップ記録）と「Phase 8 で対応」とされた findings
5. `docs/設計/<機能名>/_review/test/review-log.md` — Phase 6 の収束ログ（Critical/Major ゼロで収束していることの確認）
6. `docs/設計/<機能名>/_review/test/red-phase-evidence.md` — Red Phase 証拠
...
### Step 1: テスト承認の確認（フロー設定に応じて分岐）

まず `_flow-config.md` を読み、Phase 7（テスト承認ゲート）の設定を確認する。以下を満たさない場合は本skillを実行せず、先行フェーズの完了を案内する：

**Phase 7「実施」の場合：**

- `_review/test/findings.md` に **Phase 7 の承認判定記録** があること
- Red Phase 証拠が存在すること
- Critical / Major findings が解消済みであること（収束ログ `review-log.md` と承認記録で確認）

**Phase 7「スキップ」の場合：**

- `_review/test/review-log.md` に **Phase 6 の収束記録（Critical 0・Major 0）** があること
- `_review/test/findings.md` に「Phase 7: スキップ（フロー設定）／収束をもって承認」の記録があること
- Red Phase 証拠が存在すること
- **Major が「文書化のみで通過」扱いになっている項目が1件もないこと**（スキップ設定ではこの通過方法は認められない。1件でもあれば Phase 6 の収束ループへ差し戻す）
