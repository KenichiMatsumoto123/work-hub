...
- ステータス：**レビュー中**
- 作成日：2026-08-16
- 最終更新：2026-08-16（Phase 2。Q1〜Q12 を反映）
- 関連ドキュメント：
...
| E2E-L5 | `/` を開き読み込み完了後、日付を `2000-02-11` にする（行なし・未保存なしなので confirm は出ない）。空フォームで取引先名を 1 文字入れ、日付を `2000-02-02` に変え、`confirm` をキャンセルする。dialog の `message` は `DATE_CHANGE_CONFIRM` と完全一致。日付は `2000-02-11` のまま、取引先名の 1 文字も残る | AC-L50 |
