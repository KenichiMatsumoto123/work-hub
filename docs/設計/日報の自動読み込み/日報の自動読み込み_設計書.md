...
- ステータス：**レビュー中**
- 作成日：2026-08-16
- 最終更新：2026-08-16（Phase 2 Round 1 後。Q2・Q7 以外の指摘を反映）
- 関連ドキュメント：
...
Phase 2 Round 1 でユーザーが確定した仕様（Q2・Q7 は未決のまま）。

| # | 論点 | 決定 |
|---|---|---|
| Q1 | フォームの「一致」 | `isReportDirty` と同じフィールド集合（id 除外）。E2E は日付・始業・取引先名・実績h・良かった点を画面で見る |
| Q3 | 親 E2E の待ち | AC-L63。`effort-normalize` と `daily-report` は読み込み完了を待ってから操作する |
...
| ID | 受け入れ条件 |
|---|---|
| AC-L40 | 401 以外の失敗（通信失敗、および読み取った値が `isValidReportStructure` を満たさない場合を含む）では、バナーが `読み込みに失敗しました`（`data-testid="report-load-status"`）と「再試行」ボタン（`data-testid="report-load-retry"`）を表示する。フォーム内容は失敗前の内容を維持する（空の初期値で上書きしない） |
...
| AC-L56 | `loadStatus` が `'loading'` または `'error'` のとき、ヘッダー遷移で `window.confirm` を呼ばない（`beforeunload` は AC-L54 どおり error 中は出す） |
| AC-L63 | `effort-normalize.test.ts` と `daily-report.test.ts` は `page.goto('/')` のあと、読み込み完了（`report-load-status` が無い、または日付欄が enabled）を待ってから日付入力・ボタン断言を行う。親 E2E の期待結果（保存メッセージ・エラー色）は変えない |
...
| Q2: `raw_data.date` と要求日付の不一致時、日付欄を要求日付で上書きするか | Phase 2 | 未決（ユーザー確認中） |
| Q7: E2E-L5/L6 の起点を実今日にするか、空のセンチネル日付へ切替後にするか | Phase 2 | 未決（ユーザー確認中） |
...
| 読み込み失敗中のヘッダー遷移 | SPA の confirm は出さない。`beforeunload`（リロード・タブ閉じ）は出す |
| 読み込み失敗中の日付変更 | 日付欄は操作可能。変えたらその日を読む |

初期表示の loading 中：`data` と `baseline` はどちらも `defaultDailyReport(getToday())`。SPA の `window.confirm` は `loadStatus === 'ready'` のときだけ行う。`beforeunload` は ready かつ dirty、または error のときに出す。
...
| `applyLoadSuccess(date, report)` | `report === null` なら `emptyReport(date)`、そうでなければ `report`（**要求日付で `date` を上書きするかは Q2 未決**） |
| `shouldPreventUnload(dirty, loadStatus)` | `loadStatus === 'error'`、または（`loadStatus === 'ready'` かつ dirty）なら true |
| `isLoadableReport(value)` | `isValidReportStructure(value)` と同じ。false なら AC-L45 |
...
7. その他例外、または戻りが non-null かつ `isLoadableReport` が false なら `loadStatus = 'error'`。`data` は変えない（空で上書きしない）
...
- SPA 遷移（`/timesheet` / `/attendance`）：TanStack Router の `useBlocker`（または同等）で、`loadStatus === 'ready'` かつ dirty なら `window.confirm(LEAVE_PAGE_CONFIRM)` を出し、キャンセルなら遷移しない。同一ルート内の state 更新では動かないこと。error / loading 中の SPA 遷移では confirm しない
...
| ID | シナリオ | 対応 AC |
|---|---|---|
| E2E-L1 | センチネル `2000-02-01` に保存済み日報がある状態で `/` を開き、読み込み完了後に日付を `2000-02-01` へ変える（未保存なし）。日付欄・始業・取引先名・実績h・良かった点が保存した値と一致する | AC-L33 |
| E2E-L2 | 保存が無い `2000-02-02` へ日付を変える。日付欄は `2000-02-02`、取引先名は空、始業は `9:00` | AC-L34 |
| E2E-L3 | `/` を開く。読み込み完了後、日付欄が `page.evaluate` した `Intl.DateTimeFormat('en-US', { timeZone: 'Asia/Tokyo', year:'numeric', month:'2-digit', day:'2-digit' })` から組み立てた `YYYY-MM-DD` と一致する。JST 0:00〜8:59 の境界は単体 AC-L01〜L03 に任せ、E2E では Date を固定しない | AC-L30・AC-L04 |
| E2E-L4 | 「テンプレ保存」ボタンが無い。「日報保存」はある | AC-L61 |
| E2E-L5 | 今日の空フォームで取引先名を 1 文字入れ、日付を `2000-02-02` に変え、`confirm` をキャンセルする。dialog の `message` は `DATE_CHANGE_CONFIRM` と完全一致。日付も取引先名も変わらない。**起点を実今日にするかセンチネルにするかは Q7 未決** | AC-L50 |
| E2E-L6 | 同様に未保存ありで「工数管理」をクリックし `confirm` をキャンセルする。dialog の `message` は `LEAVE_PAGE_CONFIRM` と完全一致。URL は `/`。**起点は E2E-L5 と同じ（Q7 未決）** | AC-L51 |
| E2E-L7 | `Date` を `2000-02-10T00:00:00+09:00` に固定し、その日付の保存済み日報を投入したうえで `/` を開く。読み込み完了後、日付欄・始業・取引先名・実績h・良かった点が保存値と一致する | AC-L31 |
