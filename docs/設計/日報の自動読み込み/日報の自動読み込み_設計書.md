...
- 「保存済み日報」とは `daily_reports.raw_data` に格納された `DailyReportData` を、既存の `rowToReport` で復元したものである（`rowToReport` は本機能で変更しない）。
...
| AC-L31 | 今日の DB 行がある状態で `/` を開くと、読み込み完了後にフォーム全体がその行の `raw_data` と一致する |
| AC-L32 | 今日の DB 行が無い状態で `/` を開くと、読み込み完了後に空の初期値（日付は今日）になる |
| AC-L33 | 日付を保存済みの日 `D` に変える（未保存なし）と、完了後にフォーム全体が `D` の保存内容になる。日付欄は `D` |
...
| ID | 受け入れ条件 |
|---|---|
| AC-L50 | dirty かつ日付変更時、`window.confirm` の引数は `入力内容が保存されていません。日付を切り替えますか？`。キャンセル（`false`）なら日付欄もフォーム内容も変わらず、`getReportByDateFn` を呼ばない |
| AC-L51 | dirty かつヘッダーの「工数管理」「勤怠管理」への遷移時、`window.confirm` の引数は `入力内容が保存されていません。このページを離れますか？`。キャンセルなら URL は `/` のまま |
...
| 既存 E2E `daily-report.test.ts` | テンプレ保存ボタン断言を削除 | AC-L61 |
| 親 E2E `effort-normalize.test.ts` | 手順は維持。開いた直後は読み込み完了を待つ必要が出る | 「実装上の注意」 |
| 日報保存 / 削除 | 変更なし | — |
| 工数管理 / 勤怠管理 | 変更なし | — |
| DB スキーマ | 変更なし | — |
...
`onChange` で `nextDate` を受け取る。

1. `nextDate === data.date` なら何もしない
2. `isReportDirty(data, baseline)` が true なら `window.confirm(DATE_CHANGE_CONFIRM)`。false なら return（controlled のため `data.date` を更新しなければ入力は元に戻る）
3. `startLoad(nextDate)`
...
`effort-normalize.test.ts` は `page.goto('/')` のあと日付をセンチネルに上書きして保存する。本機能では goto 直後に今日の読み込みが走る。

- 日付欄は読み込み中 disabled なので、**読み込み完了（日付欄が enabled、または `report-load-status` が無いこと）を待ってから**日付を入力する
- 親 E2E の期待結果（保存メッセージ・エラー色）は変えない
- 親 E2E のコード変更が必要なら、本機能の実装（Phase 8）で「待ってから入力」を足す。シナリオ仕様（観点表 2 章）は変えない

既存 `daily-report.test.ts` のテンプレ保存断言は本機能で削除する（AC-L61）。フォーム要素の存在確認は残す。
...
| E2E-L5 | 今日の空フォームで取引先名を 1 文字入れ、日付を `2000-02-02` に変え、`confirm` をキャンセルする。日付も取引先名も変わらない | AC-L50 |
| E2E-L6 | 同様に未保存ありで「工数管理」をクリックし `confirm` をキャンセルする。URL は `/` | AC-L51 |
